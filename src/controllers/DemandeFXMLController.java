...
public class DemandeFXMLController implements Initializable {
     ...
     serviceDemande service_baby1=new serviceDemande();
     ...
     public void getShowPane()
    {
        ...
        for (demande p: service_baby1.Afficherbaby())
        {
            AllProducts.add(p);
        }
        int i=0;
...
//                                });
                           t.setOnMouseClicked((MouseEvent event) -> {
                                       demande.setId_courant(p3.getId());     
       try {
        Scene scene = new Scene(FXMLLoader.load(getClass().getResource("/GUI/ShowDeatilDemande.fxml")));
