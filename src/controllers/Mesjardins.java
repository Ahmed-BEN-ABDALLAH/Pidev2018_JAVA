 ...
 * @author AHMED
 */
public class Mesjardins implements Initializable {
...
int accepter = service_jar.getmesStat("accepter",User.getId_courant_user());
                int attente = service_jar.getmesStat("attente",User.getId_courant_user());
        int refuser = service_jar.getmesStat("refuser",User.getId_courant_user());
    ...
    public void getShowPane() {
        List<Jardin> mesjardins = new ArrayList();

        service_jar.Mesjardins(User.getId_courant_user()) .stream().map((j) -> {
            mesjardins.add(j);
            return j;
        }).forEachOrdered((_item) -> {
            System.out.println(p);
        });
        System.out.println(mesjardins);
        int i = 0;
        int j = 0;
        ObservableList<Pane> Panes = FXCollections.observableArrayList();

        List<Jardin> Threejardin = new ArrayList();
        for (Jardin jar : mesjardins) {
            if (i == 0) {
                Threejardin.add(jar);
                i++;
                j++;

                if (j == mesjardins.size()) {
...
//                                });
                ...
                Text telt = new Text("Etat :  ");
                Label tel = new Label(String.valueOf(j3.getEtat()));
